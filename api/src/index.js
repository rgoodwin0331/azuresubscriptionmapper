...
// ─────────────────────────────────────────────────────────────
// GET /api/accounts/{accountId} — Account detail with subscriptions + SKUs
// ─────────────────────────────────────────────────────────────
app.http('account-detail', {
  methods: ['GET'],
  authLevel: 'anonymous',
  ...
  handler: async (request, context) => {
    try {
      ...
      const accountResult = await pool.request()
        .input('accountId', sql.Int, accountId)
        .query(`
          SELECT account_id, [name] AS account_name, company_id, tenant_id, created_at
          FROM dbo.accounts
          WHERE account_id = @accountId
        `);
      ...
      // Fetch subscriptions with SKU data
      const subsResult = await pool.request()
        .input('accountId', sql.Int, accountId)
        .query(`
          SELECT 
            ...
            s.subscription_guid,
            sk.id               AS sku_id,
            sk.u_service_offering,
            sk.u_product_id,
            sk.u_qty_to_invoice,
            sk.u_recurring_amount,
            sk.u_recurring_cost
          FROM dbo.subscriptions s
          LEFT JOIN dbo.subscription_skus ss ON s.subscription_id = ss.subscription_id
          LEFT JOIN dbo.sku_msaz001 sk ON ss.msaz001_id = sk.id
          WHERE s.account_id = @accountId
          ORDER BY s.subscription_name, sk.u_service_offering, sk.u_product_id
        `);

      // Group SKUs under each subscription
      const subsMap = {};
      for (const row of subsResult.recordset) {
        const key = row.subscription_id;
        if (!subsMap[key]) {
          subsMap[key] = {
            subscription_id: row.subscription_id,
            subscription_name: row.subscription_name,
            subscription_guid: row.subscription_guid,
            skus: [],
            total_recurring: 0
          };
        }
        if (row.sku_id) {
          subsMap[key].skus.push({
            sku_id: row.sku_id,
            service_offering: row.u_service_offering,
            product_id: row.u_product_id,
            qty_to_invoice: row.u_qty_to_invoice,
            recurring_amount: row.u_recurring_amount,
            recurring_cost: row.u_recurring_cost
          });
          if (row.u_recurring_amount) {
            subsMap[key].total_recurring += parseFloat(row.u_recurring_amount);
          }
        }
      }

      const subscriptions = Object.values(subsMap);

      // Calculate totals
      const totalSkus = subscriptions.reduce((sum, s) => sum + s.skus.length, 0);
      const totalRecurring = subscriptions.reduce((sum, s) => sum + s.total_recurring, 0);
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          meta: {
            ...
            total_sku_assignments: totalSkus,
            total_recurring_monthly: totalRecurring
          }
        })
      };
